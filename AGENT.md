...
- [docs/cloudflare.md](docs/cloudflare.md) — running behind Cloudflare (wildcard subdomains, proxied vs DNS-only, with/without Traefik); composes in `deploy/cloudflare/`.
...
- **Identity & naming** (`server/auth.go`, `identity_crud.go`): tokens carry `namespace` + `role`; inline grammar `token[@namespace][:reserved]`; writable JSON store (`users_file`) for admin CRUD merged with inline/bootstrap tokens. Services are single-level `<slug>-<namespace>.<domain>` by default (one `*.<domain>` wildcard) — nested `<slug>.<namespace>.<domain>` only with `tls-mode=dns`. Reserve names atomically via the registry (placeholder → attach); `reservedSlug()` words can't be claimed.
- **Routing modes** (`routing_mode`): `subdomain` (host-routed) or `path` (`<ns>.<domain>/<slug>/`). Registry + service store key by a **route key** (host, or `<ns>.<domain>/<slug>`); a `reservation{Key,Host,Slug,URL}` carries both. Path mode strips the slug prefix, sets a `tn_route` affinity cookie for prefix-less requests, and serves hub/`/api`/static under reserved paths. Both modes share one client + token set — the server decides addressing.
- **Control plane on the edge**: the edge also routes `<control_host>.<domain>` (default `connect`) to the control WebSocket, so clients use `wss://connect.<domain>` over the edge's TLS (single `:443` — works behind Cloudflare-proxied / any L7). The dedicated `control_addr` listener (`:7000`) still runs for behind-proxy/L4 setups. `connect` is a reserved slug.
- **DNS-01 providers kept minimal** (`internal/server/dnsacme.go`): only `cloudflare` + `digitalocean` (lightweight) are built in — the heavy AWS/GCP SDKs are excluded to keep the binary ~13 MB stripped. Add a provider import only with a clear size justification.
- **Reliability / ops**: `tunnel healthcheck` probes `/healthz` (used as the Docker `HEALTHCHECK`, distroless-friendly). Graceful shutdown via signal context + `Server.Shutdown`. Deploy variants live in `deploy/` (quickstart/standalone/file-tls/traefik/traefik-external compose + systemd unit) — keep them working and add a healthcheck/restart/persistent-volume story to new ones.
